...
public class WeeklyAppWidgetProvider extends AppWidgetProvider {
...
//	       Log.w(">>>>>>>>>>>>>", "onReceive!! Weekly");
	       ...
	       { 
	         ...
	         mSender = PendingIntent.getBroadcast(context, ComConstant.WIDGET_WEEKLY_REQ, intent, 0); 
...
//    	Log.d(TAG, "onDisabled");
    ...
    { 
//    		Log.w(">>>>>>>>>>>>>>>>>>>>>>>>>>", "Start WeeklyAppWIdget Widget!!");
  	
    		//위젯 클릭시 링크되는 class : main class link
	        RemoteViews updateViews = new RemoteViews(context.getPackageName(),  R.layout.widget_weekly_4x2); 
	        Intent intent = null;
	        //ARM 로직적용(유료버전만)
	        ...
	        //2.주간일정 화면 만들기 (일일일정 Include)
	        Calendar iCal = Calendar.getInstance();
	        updateViews.removeAllViews(R.id.lin_section1);
	        updateViews.removeAllViews(R.id.lin_section2);
	        
	        //첫번쨰칸 
	        String today = SmDateUtil.getDateFromCal(iCal);
	        updateViews.addView(R.id.lin_section1, new RemoteViews(context.getPackageName(),  R.layout.widget_weekly_today));
			updateViews.setTextColor(R.id.yyyymm, 	mFontColor);
			updateViews.setTextColor(R.id.dd, 		mFontColor);	        
